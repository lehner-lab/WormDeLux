...
import org.kuleuven.streams.BinaryInputStream;
import org.kuleuven.visitor.Visitable;
import org.kuleuven.visitor.Visitor;

public abstract class Vector<D> implements Visitable {
	...
	public double squaredDistanceTo(Vector<D> vector) {
		/*
		 * double result = 0; double distance;
		 * 
		 * for (Object dimension: getSpace()) { distance = get(dimension) -
		 * vector.get(dimension); result += distance * distance; }
		 * 
		 * return result;
		 */
		...
		double result = vectorSums.sumXX - 2 * vectorSums.sumXY
				+ vectorSums.sumYY;

		if (result < 0d) // Catch unstability: in theory 'result' should be
							// nonnegative
			return 0d;
		else
			return result;
	...
	public double manhattanDistanceTo(Vector<D> vector) {
		ParallelVectorCursor<D> cursor = getParallelNonzeroCursor(vector);
		double result = 0;
		while (cursor.isValid()) {
			result += Math.abs(cursor.lhs.get() - cursor.rhs.get());
			cursor.next();
		}
	...
	/**
	 * Adjusts the length of the vector in such a way that the maximum value is
	 * 1.
	 */
	public void unify() {
		divide(max());
	...
	public static <D> SparseVector<D> pairwiseElementwiseProduct(
			List<? extends Vector<D>> vectors) {
	...
	public ParallelVectorCursor<D> getParallelCursor(Vector<D> vector) {
		return new ParallelVectorCursor<D>(getCursor(),
				vector.getSlaveCursor(), false);
	}

	public ParallelVectorCursor<D> getParallelNonzeroCursor(Vector<D> vector) {
		if (getStoredValueCount() <= vector.getStoredValueCount())
			return new ParallelVectorCursor<D>(getNonzeroCursor(),
					vector.getSlaveCursor(), false);
		else
			return new ParallelVectorCursor<D>(vector.getNonzeroCursor(),
					getSlaveCursor(), true);
	}
	...
	public String getNonzerosString() {
		...
		while (cursor.isValid()) {
			stringBuffer
					.append(cursor.dimension() + ": " + cursor.get() + "\n");
			cursor.next();
		}
	...
	public void add(Vector<D> vector) {
		ParallelVectorCursor<D> cursor = new ParallelVectorCursor<D>(
				vector.getNonzeroCursor(), getSlaveCursor(), false);

		while (cursor.isValid()) {
			cursor.slaveCursor.set(cursor.slaveCursor.get()
					+ cursor.masterCursor.get());
			cursor.next();
		}
		squarednorm = -1d;
	}
	...
	public static <D> void meanVector(Vector<D> result,
			Collection<? extends Vector<D>> vectors) {
		for (Vector<D> vector : vectors) {
			ParallelVectorCursor<D> cursor = new ParallelVectorCursor<D>(
					vector.getNonzeroCursor(), result.getSlaveCursor(), false);
...
/**
 * this works looping over all dimensions, does not work for HD sparse vectors
 * @param result
 * @param vectors
 */
	public static <D> void medianVector(Vector<D> result,
			Collection<? extends Vector<D>> vectors) {
		VectorCursor<D> cursor =result.getCursor();
		while(cursor.isValid()){
			double[] values= new double[vectors.size()];
			int i =0;
			for(Vector<D> v:vectors){
				values[i++]=v.get(cursor.dimension());
			}
			cursor.set(MathEMC.median(values));
			cursor.next();
		}
	}
	 ...
	 * 
	 * Note: Is only based on vector components where one or both vectors have a
	 * non-zero value!
	 * 
	 * @param Other
	 *            vector
	...
	public double pearsonCorrelation(Vector<D> vector) {
		...
		double inbetween = Math
				.sqrt((vectorSums.sumXX - (vectorSums.sumX * vectorSums.sumX)
						/ doubleN)
						* ((vectorSums.sumYY - (vectorSums.sumY * vectorSums.sumY)
								/ doubleN)));
		...
		return (vectorSums.sumXY - (vectorSums.sumX * vectorSums.sumY)
				/ doubleN)
				/ inbetween;
	...
	public double kendallsTau(Vector<D> vector) {
		...
		double result = is / (Math.sqrt(n1) * Math.sqrt(n2));
		return result;
	}

	public double spearmanRankCorrelation(Vector<D> vector) {
		// Note: this correlation factor should only be used when there are not
		// a lot of ties!
		...
		while (cursor1.isValid()) {
			master.add(new ObjectAndDoubleEntry<D>(cursor1.masterCursor
					.dimension(), cursor1.masterCursor.get()));
			slave.add(new ObjectAndDoubleEntry<D>(cursor1.slaveCursor
					.dimension(), cursor1.slaveCursor.get()));
			cursor1.next();
		}
		int check = 0;
		while (cursor2.isValid()) {
			if (cursor2.slaveCursor.get() == 0) {
				slave.add(new ObjectAndDoubleEntry<D>(cursor2.masterCursor
						.dimension(), cursor2.masterCursor.get()));
				master.add(new ObjectAndDoubleEntry<D>(cursor2.slaveCursor
						.dimension(), cursor2.slaveCursor.get()));
			} else {
				check++;
			}
			cursor2.next();
		...
		Collections.sort(master,
				ObjectAndDoubleEntry.mapEntryComparatorDescending());
		...
		// calculating ranks taking ties into account (averaging ranks when ties
		// occur)
		// once
		int i = 0;
		...
		while (i < master.size()) {
			...
			int number = 1;
			while (j < master.size()
					&& (master.get(i).value == master.get(j).value)) {
				rank += j + 1d;
				number++;
				j++;
			}
			if (number > 1)
				sum0fTies3MinusTies1 += (number) * (number) * (number)
						- (number);
		...
		while (i < slave.size()) {
			...
			int number = 1;
			while (j < slave.size()
					&& (slave.get(i).value == slave.get(j).value)) {
				rank += j + 1d;
				number++;
				j++;
			}
			if (number > 1)
				sum0fTies3MinusTies2 += (number) * (number) * (number)
						- (number);
		...
		double fac = (1 - (double) sum0fTies3MinusTies1 / (double) N3minusN)
				* (1 - (double) sum0fTies3MinusTies2 / (double) N3minusN);
		ParallelVectorCursor<D> cursor = resultLhs
				.getParallelNonzeroCursor(resultRhs);
		double sumOfSquaredDifferenceInRanks = 0d;
		while (cursor.isValid()) {
			double difference = cursor.masterCursor.get()
					- cursor.slaveCursor.get();
			sumOfSquaredDifferenceInRanks += difference * difference;
			cursor.next();
		}
		return (1 - (6 / (double) N3minusN)
				* (sumOfSquaredDifferenceInRanks + ((double) sum0fTies3MinusTies1 + (double) sum0fTies3MinusTies2) / 12d))
				/ Math.sqrt(fac);
	}
	...
	public static <D> SparseVector<D> elementwiseProduct(Vector<D> lhs,
			Vector<D> rhs) {
	...
	private List<ObjectAndDoubleEntry<D>> getNonzeroEntrieslist() {
		...
		while (vectorcursor.isValid()) {
			values.add(new ObjectAndDoubleEntry<D>(vectorcursor.dimension(),
					vectorcursor.get()));
			vectorcursor.next();
		}
		return values;
	...
	public List<ObjectAndDoubleEntry<D>> getNonZeroEntriesSortedOnAscendingValue() {
		...
		Collections.sort(values,
				ObjectAndDoubleEntry.mapEntryComparatorAscending());
		return values;
	}
